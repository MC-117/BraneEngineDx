...
string configPath;
filesystem::path workingSpace;

void main(int argc, char** argv) {
	if (argc > 1) {
		workingSpace = filesystem::u8path(argv[1]);
		if (filesystem::exists(workingSpace))
			filesystem::current_path(workingSpace);
	}
	Engine::config();
	Engine::setup();
	Engine::start();
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	if (__argc > 1) {
		workingSpace = filesystem::u8path(__targv[1]);
		workingSpace = filesystem::absolute(workingSpace);
		if (filesystem::exists(workingSpace))
			filesystem::current_path(workingSpace);
	}
	//try {
...
void Engine::setup()
{
	...
	CoInitialize(0);
	{
		...
		windowContext.workingPath = workingSpace.generic_u8string();
	}
