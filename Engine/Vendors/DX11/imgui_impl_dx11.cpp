...
#include "../../Core/WUI/WUIImGuiWindow.h"
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data, ID3D11DeviceContext* device_context)
{
    ...
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    ID3D11DeviceContext* ctx = device_context;
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        // Create the ps constant buffer
        {
            ...
            desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
            desc.MiscFlags = 0;
            bd->pd3dDevice->CreateBuffer(&desc, NULL, &bd->pPixelConstantBuffer);
        }
    }
...
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    WUIImGuiWindow* window = (WUIImGuiWindow*)viewport->PlatformUserData;
    IM_ASSERT(window != 0);
    IDeviceSurface* deviceSurface = (IDeviceSurface*)viewport->RendererUserData;
    IM_ASSERT(deviceSurface != 0);
    //deviceSurface->resize(viewport->Size.x, viewport->Size.y);
...
static void ImGui_ImplDX11_RenderWindow(ImGuiViewport* viewport, void* rawContext)
{
    IM_ASSERT(rawContext != 0);
    DX11RenderContext& context = *(DX11RenderContext*)rawContext;
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    IDeviceSurface* deviceSurface = (IDeviceSurface*)viewport->RendererUserData;
    IM_ASSERT(deviceSurface != 0);
    ...
    if (!(viewport->Flags & ImGuiViewportFlags_NoRendererClear))
        context.clearSurfaceColor(clear_color);
    ImGui_ImplDX11_RenderDrawData(viewport->DrawData, context.deviceContext.Get());
}
