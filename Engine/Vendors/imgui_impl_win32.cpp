...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE: case WM_NCMOUSEMOVE:
    {
        ...
        if (msg != WM_NCMOUSEMOVE && io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
...
{
    WUIImGuiWindow* window = new WUIImGuiWindow();
    window->initViewport(*viewport);
    window->create();
}
...
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    if (WUIImGuiWindow* window = (WUIImGuiWindow*)viewport->PlatformUserData)
    {
        if (::GetCapture() == window->getHWnd())
        {
...
static void ImGui_ImplWin32_ShowWindow(ImGuiViewport* viewport)
{
    ...
    IM_ASSERT(window->getHWnd() != 0);
    window->show(!(viewport->Flags & ImGuiViewportFlags_NoFocusOnAppearing));
}
...
static bool ImGui_ImplWin32_GetWindowFocus(ImGuiViewport* viewport)
{
    WUIImGuiWindow* window = (WUIImGuiWindow*)viewport->PlatformUserData;
    IM_ASSERT(window->getHWnd() != 0);
    return ::GetForegroundWindow() == window->getHWnd();
}
...
static bool ImGui_ImplWin32_GetWindowMinimized(ImGuiViewport* viewport)
{
    WUIImGuiWindow* window = (WUIImGuiWindow*)viewport->PlatformUserData;
    IM_ASSERT(window->getHWnd() != 0);
    return ::IsIconic(window->getHWnd()) != 0;
}
...
static float ImGui_ImplWin32_GetWindowDpiScale(ImGuiViewport* viewport)
{
    WUIImGuiWindow* window = (WUIImGuiWindow*)viewport->PlatformUserData;
    IM_ASSERT(window->getHWnd() != 0);
    return ImGui_ImplWin32_GetDpiScaleForHwnd(window->getHWnd());
}
