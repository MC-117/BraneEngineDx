...
void ax::NodeEditor::Detail::EditorContext::BeginLinkEx()
{
    if (auto drawList = ImGui::GetWindowDrawList())
    {
        //m_CurrentChannel = drawList->_Splitter._Current;
        //drawList->ChannelsSetCurrent(c_LinkChannel_Links);
    }
}

void ax::NodeEditor::Detail::EditorContext::EndLinkEx()
{
    if (auto drawList = ImGui::GetWindowDrawList())
    {
        //drawList->ChannelsSetCurrent(m_CurrentChannel);
    }
}
...
void ax::NodeEditor::Detail::EditorContext::Flow(Node* node)
{
    m_FlowAnimationController.Flow(node);
}
...
void ax::NodeEditor::Detail::FlowAnimation::Flow(Node* node, float markerDistance, float speed, float duration)
{
    Stop();
...
{
    if (!IsPlaying() || m_Object == NULL || !m_Object->IsVisible())
        return;
    ...
    Node* m_Node = m_Object->AsNode();
    if (m_Node) {
        m_Node->DrawBorder(drawList, flowColor, 5.0f);
    }
}
...
bool ax::NodeEditor::Detail::FlowAnimation::IsNodeValid() const
{
    Node* m_Node = m_Object->AsNode();
    return m_Node && m_Node->m_IsLive;
}
...
bool ed::FlowAnimation::IsPathValid() const
{
    ...
    Node* m_Node = m_Object->AsNode();
    if (m_Node)
        return m_Path.size() == 0 && m_PathLength == 0.0f;
}

void ed::FlowAnimation::UpdatePath()
{
    if (!IsLinkValid() && !IsNodeValid())
    {
        ClearPath();
        return;
    }
...
void ax::NodeEditor::Detail::FlowAnimationController::Flow(Node* node)
{
    ...
    auto animation = GetOrCreate(node);

    animation->Flow(node, editorStyle.FlowMarkerDistance, editorStyle.FlowSpeed, editorStyle.FlowDuration);
}
...
ed::FlowAnimation* ed::FlowAnimationController::GetOrCreate(Object* object)
{
    // Return live animation which match target link
    {
        auto animationIt = std::find_if(m_Animations.begin(), m_Animations.end(), [object](FlowAnimation* animation) { return animation->m_Object == object; });
