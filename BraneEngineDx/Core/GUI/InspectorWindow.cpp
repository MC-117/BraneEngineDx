...
void InspectorWindow::onRenderWindow(GUIRenderInfo& info)
{
	...
	if (target != NULL) {
		Object* pObj = dynamic_cast<Object*>(target);
		IWorld* iWorld = NULL;
		if (pObj != NULL) 
			while (iWorld == NULL && pObj->parent != NULL) {
				iWorld = dynamic_cast<IWorld*>(pObj);
				pObj = pObj->parent;
			}
		if (iWorld == NULL) {
...
//				}
//
//				RigidBody* rigid = tf->rigidBody;
//				if (rigid != NULL && ImGui::CollapsingHeader("Collision")) {
//					static const char* btStr[] = { "NONE", "RIGID", "SOFT" };
//					static const char* ctStr[] = { "NONE", "RIGID", "SOFT" };
//					ImGui::Text("BodyType: %s", btStr[rigid->bodyType]);
//					ImGui::Text("ComplexShape: %s", rigid->shapeComplexType == SIMPLE ? "SIMPLE" : "COMPLEX");
//
//					Vector3f pos = rigid->getPositionOffset();
//					if (ImGui::DragFloat3("Position##Collision_Position", pos.data(), 0.01)) {
//						rigid->setPositionOffset(pos);
//					}
//
//					Quaternionf rot = rigid->getRotationOffset();
//					Vector3f rot_eular = rot.toRotationMatrix().eulerAngles();
//					if (ImGui::DragFloat3("Rotation##Collision_Rotation", rot_eular.data(), 0.01)) {
//						rigid->setRotationOffset(rot.setFromEularAngles(rot_eular));
//					}
//
//					if (rigid->shape != NULL) {
//						if (ImGui::TreeNode((rigid->shape->getSerialization().type + " Shape").c_str())) {
//							ImGui::InputFloat3("MinPoint", rigid->shape->bound.minVal.data(), "%.3f", ImGuiInputTextFlags_ReadOnly);
//							ImGui::InputFloat3("MaxPoint", rigid->shape->bound.maxVal.data(), "%.3f", ImGuiInputTextFlags_ReadOnly);
//							ImGui::TreePop();
//						}
//					}
//				}
...
//								shp->getHeight(),
//								shp->getDepth());
//							MeshRender* mrd = dynamic_cast<MeshRender*>(rd);
...
//											bool enable = mrd->getPartEnable(b->second);
//											if (ImGui::Checkbox("Enable", &enable))
//												mrd->setPartEnable(b->second, enable);
//
...
//						}
//						else {
//							string meshPath = MeshAssetInfo::getPath(mR->mesh);
//							ImGui::Text(meshPath.c_str());
//						}
//
//						if (assignAsset != NULL && assignAsset->assetInfo.type == "Mesh") {
//							if (ImGui::Button("Set Selected Mesh", { -1, 40 })) {
//								mR->setMesh((Mesh*)assignAsset->load());
//							}
//						}
//
//						if (mR->mesh != NULL) {
...
//						continue;
//					unsigned long long id = ((Texture2D*)_b->second->asset[0])->getTextureID();
//					if (id == 0)
//						id = Texture2D::blackRGBDefaultTex.getTextureID();
