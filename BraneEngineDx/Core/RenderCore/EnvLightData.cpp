...
{
	EnvLightData& data = envLightdata.emplace_back(EnvLightData());
	...
	data.set(capture->shCoeff3RGB);
	return envLightdata.size() - 1;
}

void EnvLightRenderData::create()
{
	int count = envLightdata.size();
	for (int i = 0; i < count; i++) {
		envLightdata[i].reverseIndex = count - i - 1;
	}
}

void EnvLightRenderData::release()
{
	envLightDataBuffer.resize(0);
}

void EnvLightRenderData::upload()
{
	envLightDataBuffer.uploadData(envLightdata.size(), envLightdata.data());
}
