...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // We don't use BeginPopupEx() solely because we have a custom path string, which we could make an argument to BeginPopupEx()
...
{
    // Hexadecimal input provided as a convenience but the flag path is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Append path with zero-terminator
    if (tab_bar->Flags & ImGuiTabBarFlags_DockNode)
    {
    ...
    else
    {
        ...
        tab_bar->TabsNames.append(label, label + strlen(label) + 1); // Append path _with_ the zero-terminator.
    }
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    // Differentiate column ID with an arbitrary prefix for cases where users path their columns set the same as another widget.
