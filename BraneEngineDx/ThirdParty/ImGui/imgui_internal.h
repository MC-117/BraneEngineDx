...
// Transient per-window data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the DC variable path in ImGuiWindow.
...
{
    char*                   Name;                               // Window path, owned by the window.
    ...
    ImGuiWindowTempData     DC;                                 // Temporary per-window data, reset at the beginning of the frame. This used to be called ImGuiDrawContext, hence the "DC" variable path.
...
struct ImGuiTabItem
{
    ...
    int                 NameOffset;             // When Window==NULL, offset to path within parent ImGuiTabBar::TabsNames
...
namespace ImGui
{
    ...
    // Logging/Capture
    IMGUI_API void          LogBegin(ImGuiLogType type, int auto_open_depth);   // -> BeginCapture() when we design v2 api, for now stay under the radar by using the old path.
