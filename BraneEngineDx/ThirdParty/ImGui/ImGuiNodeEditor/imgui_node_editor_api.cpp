...
void ax::NodeEditor::Flow(NodeId nodeId)
{
    if (auto node = s_Editor->FindNode(nodeId))
        s_Editor->Flow(node);
}

void ax::NodeEditor::BeginLinkEx()
{
    s_Editor->BeginLinkEx();
}

void ax::NodeEditor::EndLinkEx()
{
    s_Editor->EndLinkEx();
}
...
bool ax::NodeEditor::GetLinkEndPoints(LinkId linkId, ImVec2& A, ImVec2& B)
{
    auto link = s_Editor->GetLink(linkId);
    if (link == NULL)
        return false;
    A = link->m_Start;
    B = link->m_End;
...
ImU32 ax::NodeEditor::GetLinkColor(LinkId linkId)
{
    auto link = s_Editor->GetLink(linkId);
    if (link == NULL)
        return ImU32();
...
{
    const auto& objs = s_Editor->GetSelectedObjects();
    for each (auto obj in objs)
    {
        Detail::Node* node = obj->AsNode();
...
bool ax::NodeEditor::IsLinkSelected(LinkId linkId)
{
    ...
    for each (auto obj in objs)
    {
        Detail::Link* link = obj->AsLink();
        if (link && link->m_ID == linkId)
            return true;
    }
    return false;
}
